...
class Assets
{
     ...
     * @param array $options URL options. Available values: width, height, cacheMaxAge, quality, robotsNoIndex, download, svgFill, svgStroke, rotate, cropX, cropY, cropWidth, cropHeight, version and outputType.
    ...
    public function getURL(string $filename, array $options = []): string
    {
        ...
        if ($url === null) {
            ...
            if (isset($options['version'])) {
                $optionsString .= '-v' . substr(md5(md5($filename) . (is_string($options['version']) ? $options['version'] : serialize($options['version']))), 0, 10);
            }
            if (isset($options['outputType']) && isset($pathInfo['extension'])) {
                if (!is_string($options['outputType'])) {
                    throw new \InvalidArgumentException('The value of the outputType option must be of type string, ' . gettype($options['outputType']) . ' given.');
                }
     ...
     * 
     * @param string $filename The filename.
     * @param array $options List of options. Available values: width, height, quality and outputType.
     * @throws \InvalidArgumentException
    ...
    public function isSupportedOutputType(string $name): bool
    {
        ...
        if (!isset($this->cache[$cacheKey])) {
            ...
            $this->cache[$cacheKey] = array_search($name, $types) !== false;
        }
        return $this->cache[$cacheKey];
    }
     ...
     * @param array $options Modify options. Available values: width, height, quality, rotate, cropX, cropY, cropWidth and cropHeight.
     * @throws \InvalidArgumentException
     * @throws \Exception
